...
const SignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const request = await apiClient.post("/auth/signup", {
      ...
      if(request.status === 201){
        ...
        navigate(`/verify/${request.data.user._id}?email=${email}`);
        console.log({request});
      }
    } catch (error) {
      const axiosError = error as AxiosError<{ message: string }>;
      const errorMessage = axiosError.response?.data?.message || 'An error occurred during signup';
      toast.error(errorMessage);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <Helmet>
        <title>Sign Up - Modulive</title>
        <meta name="description" content="Create your Modulive account" />
      </Helmet>
        ...
        >
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-400">
              Join us and start shopping for beautiful furniture
            </p>
          </div>
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            ...
            {/* <Button
              ...
              size="lg"
              className="w-full"
              onClick={() => toast.success('Google Sign-Up would be implemented here')}
            >
